...
# # Data
# data = {
#     "Scenario": ["Grid Only", "Base Case", "SS75%"],
#     "Maximal Import Peak in kW": [613.57, 562.67, 410.73]
# }
...
# plt.xticks(x, df['Scenario'], fontsize=20)  # Ensure scenario names are shown
# plt.yticks(fontsize=20)
...
# # Convert power values from Watts to kilowatts
# df_optimization_results[['P_demand', 'P_imp', 'P_PV', 'P_exp']] /= 1000

# # Resample data to daily sums
# daily_data = df_optimization_results[['P_demand', 'P_imp', 'P_PV', 'P_exp']].resample('D').sum()
...
#     "low efficiencies high costs w/o Leistungstarif": [63.07, 63.07, 63.07, 182.03],
...
# markers = ['o', '^', 's', 'P', '*']  # Different markers for each line

# for i, column in enumerate(df.columns[1:]):
#     sns.lineplot(x=df["Self-Sufficiency in %"], y=df[column], marker=markers[i], markersize=12, linewidth=4) # ,label=column
