...
def export_optimization_results(variable_names, variable_values, results_directory):
    """
    ...
    optimization_status = variable_values.get('status', None)

    # Check if the optimization was successful
    if optimization_status == GRB.OPTIMAL:
        # Create a DataFrame with the results
        results = pd.DataFrame({name: variable_values[name] for name in variable_names})
