...
def plot_power_generation(P_PV, P_imp, P_exp, df_input):
    ...
    axs[0].plot(range(len(df_input)), [P_PV[t]  / 1000 for t in range(len(df_input))], label='PV Generation', color='orange')
    axs[0].plot(range(len(df_input)), [P_imp[t] / 1000 for t in range(len(df_input))], label='Imported Power', color='blue')
    axs[0].set_ylabel('Power [kW]')
    axs[0].legend(loc='upper left')
    axs[0].set_title('PV Generation and Imported Power')

    axs[1].plot(range(len(df_input)), [P_exp[t] / 1000 for t in range(len(df_input))], label='Exported Power', color='green')
