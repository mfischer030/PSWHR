...
# cooling_demand = df_demand_heat['Cooling_10degC_kW'].values  # Cooling demand in [kW]
...
# fig.suptitle('Heat Demand 2022 NEST')

# # Plot Cooling demand
# axs[0].fill_between(range(len(cooling_demand)), cooling_demand, label='Cooling Demand', color='blue', alpha=0.4)
# axs[0].set_ylabel('Demand (kW)')
# axs[0].legend()

# # Plot Heating demand at 35°C
# axs[1].fill_between(range(len(heat_35degC_demand)), heat_35degC_demand, label='Heating Demand 35°C', color='orange', alpha=0.4)
# axs[1].set_ylabel('Demand (kW)')
# axs[1].legend(loc='upper right')

# # Plot DHW demand at 65°C
# axs[2].fill_between(range(len(heat_65degC_demand)), heat_65degC_demand, label='Heat Demand 65°C', color='red', alpha=0.4)
# axs[2].set_ylabel('Demand (kW)')
# axs[2].legend()
...
# eta = {'PV': 0.21,'ELY': 0.8,'C': 1,'TANK': 1,'FC': 1}
# eta = {'PV': 0.21,'ELY': 1,'C': 1,'TANK': 1,'FC': 1}            # Optimal      

# Unit prices of components / capital costs in [€/W] => from Roxanne-----------
# PV:   in [€/W] | 2023_Tya Son Le: 881 USD/year | 2018_Gabrielli 300€/m2 
# ELY:  in [€/W] | 2023_IEA-GlobalH2Review: PEM - 2kUSD/kW - reduction to 600 USD/kW
# C:    in [€/W] | 2020_Pan et al: 1228 (¥/kW)
# TANK: in [EUR/J], 1644 [EUR/kgH2] | 2018_Gabrielli: {20.7;13.6;10.9} in [€/kWh]
# FC:   in [€/W] (Wang et. al 2023 - 2kUSD/kW) | 2018_Gabrielli {2160;1680;1320} [€/kW]


UP = {'PV': 0.8,'ELY': 1.7,'C': 0.0076,'TANK': 1644/HHV,'FC': 1.680}            # Realistic

#UP = {'PV': 0.8,'ELY': (1.7+0.556)/2,'C': 0.0076,'TANK': 1644/(1.5*HHV),'FC': (1.68+0.556)/2}

#UP = {'PV': 0.8,'ELY': 0.556,'C': 0.0076/2,'TANK': 1644/(3*HHV),'FC': 0.556}    # Optimal
...
Area_PV_max  = 3000 # Maximum PV area [m2]

# Electrolyser max and min nominal power (W)   
S_ELY_max = 400*1000   # Maximal Electrolyzer size in [W]
...
S_TANK_max = E_demand_day * 7 * 3600                    # E_demand_day in [Wh]  #Gabriele: E_demand_day is in [Wh], correct? why do you multiply for deltat? MAxime: to convert Wh in J (1 Wh = 3600 J)
...
if scenario_choice == 'grid':
    ...
    P_exp_ub = GRB.INFINITY            # Upper bound for P_exp in grid-connected scenario, previously S_FC_max 
...
# m.addConstrs((P_imp[t] <= 0.4*P_demand[t] for t in range(nHours)),  name= "grid25")
    

# Initializing FC power
m.addConstr(P_FC[0]   <= E_TANK[0] / deltat)                                    
...
LCOE = (cost - cost_startup) / (sum(P_imp + P_PV)/10**6)  # Calculate LCOE
