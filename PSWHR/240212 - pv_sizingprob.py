...
project_lifetime = 25        # in [y] - variable is not used for the moment...
...
#        'ELY': 0.6,     # 
#        'C': 0.7526,    # from Roxanne | 2020_Pan et al: 0.9
...
UP = {
    'PV': 0.8,                # in [€/W] | 2023_Tya Son Le: 881 USD/year | 2018_Gabrielli 300€/m2 
    'ELY': 1.7/10,            # in [€/W] | 2023_IEA-GlobalH2Review: PEM - 2kUSD/kW
    'C': 0.0076,              # in [€/W] | 2020_Pan et al: 1228 (¥/kW)
...
S_ELY_min = 250*1000   # Min. size ELY where problem is feasible [W] - from Rox
...
S_TANK_max = E_demand_day * 4                           # E_demand_day in [Wh]  #Gabriele: E_demand_day is in [Wh], correct? why do you multiply for deltat? 
...
if [mdot_H2[t] > 0 for t in range(nHours)]: 
    LCOH = (cost - cost_startup) / (sum(mdot_H2)*3600) # From Roxanne 
    print("LCOH =", LCOH, "euro per kilogramm hydrogen")
"""
