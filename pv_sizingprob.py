...
life = {'PV': 25,'ELY': 20, 'C': 20, 'TANK': 35, 'FC': 20}
...
E_demand_day = sum(P_demand)/(4*days)
...
S_TANK_max = E_demand_day * 4 * deltat                  # E_demand_day in [Wh]
...
P_exp   = PV_sizingprob.addVars(nHours, lb=0, ub=max(P_ELY),   name="P_exp")   # Check upper-bound

#------------------------------------------------------------------------------
# Additional variables definition
# maximum PV size needs to be defined to define the upper bound of the design variable.
P_PV = [irradiance[t] * eta['PV'] * Area_PV for t in range(nHours)]  # Hourly PV power generation [W]
P_PV_peak = max(irradiance) * eta['PV'] * Area_PV  # Peak power for investment cost [W]
S_PV = P_PV_peak

# Hydrogen mass flow
# Nominal mass flow hydrogen [kg/s]
...
#------------------------------------------------------------------------------
# Constraints
PV_sizingprob.addConstr(E_TANK[0] == 0)

for t in range(1, nHours):
    # constraint for the energy balance in time over the storage component
    PV_sizingprob.addConstr(E_TANK[t] == E_TANK[t-1] + P_ELY[t] * eta['ELY'] * deltat - P_FC[t] * deltat / eta['FC'])
    # constraint for the ELY power not to excedd the storage capacity
    PV_sizingprob.addConstr(P_ELY[t]  <= (S_TANK - E_TANK[t-1]) / deltat) 
    # constraint for the FC to only use hydrogen available in the storage
    PV_sizingprob.addConstr(P_FC[t]   <= E_TANK[t-1] / deltat)
    PV_sizingprob.addConstr(P_FC[0]   <= E_TANK[0] / deltat) 
...
PV_sizingprob.addConstrs((P_ELY[t] + P_C[t] + P_exp[t] + P_demand[t] <= P_PV[t] + P_imp[t] + P_FC[t]  for t in range(nHours)), name='EnergyBalance')  
