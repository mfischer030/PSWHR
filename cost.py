...
def totalAnnualCost(system_sizes, energy_tariff, discountRate,
   ...
   #  cost_inst, cost_maint = calculate_total_costs(costs_with_annuity)
    ...
    def electricity_cost(energy_tariff, P_imp, P_max_imp, P_exp, m, high_usage, df_input, timeline_choice, vat_included=True):
        ...
        cost_elec_exp           = 0*cost_import_peak
        cost_taxes              = 0*cost_import_peak
        cost_elec_imp           = 0*cost_import_peak
        cost_grid_usage         = cost_import_peak                               # cost_grid_usage = Arbeitstarif (per kWh) + Leistungstarif (per kW/month)
    ...
    cost_WHR = (0)*sum((P_th_LT[t]/1000 * cost_export_heatLT + P_th_HT[t]/1000 * cost_export_heatHT) for t in range(nHours))
