...
tau_ph = 2.00;            % Air pre-heater time constant                   [s]

% MISCELLANEOUS AUXILIARIES
P_gamma = 150.0;          % Heat-up/Miscellaneous losses                   [W]
...
% REFORMING - Data from [5] and Aspen Simulations
split_CH4  = 0.30;          % Additional fuel for the reforming             
T_ref      = 114 + 273;     % Reformer temperature                           [CV]
eta_ref    = 0.98;          % Reformer efficiency
eta_ref_th = 0.75;          % Reformer thermal efficiency
CV         = 4 * eta_ref;   % CH4 to H2 conversion factor
tau1_ref   = 2.00;          % Time constant of the reformer                [s]
tau2_ref   = 2.00;          % Time constant of the reformer                [s]
...
% REFORMING HEAT OF REACTION
h_CH4ref = -206000;               % Spec. heat absorbed by reforming       [J / mol]
h_COref  = 41000;                 % Spec. heat produced by WGS             [J / mol]
h_ref    = - h_CH4ref - h_COref;  % Specific heat absorbed by reforming    [J / mol]

% REFORMING PRE-HEATING
SC       = 2.50;                  % Steam to carbon ratio
T_H2Oout = 973.15;                % Water temperature in the reformer
T_CH4out = 973.15;                % Fuel temperature in the reformer

% H2O specific inlet enthalpy [J / g]
h_H2Oref_in = refpropm('H', 'T', T_amb, 'P', p_an * 100, 'WATER.FLD') ...
    / 1000;

% H2O specific outlet enthalpy [J / g]
h_H2Oref_out = refpropm('H', 'T', T_H2Oout, 'P', p_an * 100, ...
    'WATER.FLD') / 1000;

% H2O specific enthalpy variation [J / mol]
h_H2Oref = (h_H2Oref_out - h_H2Oref_in) / 0.97 * M_H2O;

% CH4 specific inlet enthalpy [J / g]
h_CH4in = refpropm('H', 'T', T_amb, 'P', p_an / 1000, 'METHANE.FLD') / 1000;

% CH4 specific outlet enthalpy [J / g]
h_CH4out = C_CH4ref * T_CH4out;

% CH4 specific enthalpy variation [J / mol]
h_CH4ref = (h_CH4out - h_CH4in) / 0.97 * M_CH4;

% HEAT RECOVERY FROM FUEL PROCESSOR
% Enthalpy at fuel processor outlet [J / kg]
h_CO2rec_in = refpropm('H', 'T', T_ref, 'P', p_an * 100, 'CO2.FLD');
h_H2rec_in  = refpropm('H', 'T', T_ref, 'P', p_an * 100, 'HYDROGEN.FLD');
C_H2Orec    = refpropm('C', 'T', T_ref, 'P', p_an * 100, 'WATER.FLD');

% Enthalpy at cell inlet [J / kg]
h_CO2_T = refpropm('H', 'T', T, 'P', p_an * 100, 'CO2.FLD');
h_H2_T  = refpropm('H', 'T', T, 'P', p_an * 100, 'HYDROGEN.FLD');

% Recovered heat [J / mol]
h_CO2rec = (h_CO2rec_in - h_CO2_T) / 1000 * M_CO2 * eta_ref;
h_H2rec  = (h_H2rec_in - h_H2_T) / 1000 * M_CO2 * eta_ref;
h_H2Orec = C_H2Orec * (T_ref - T) / 1000 * M_CO2 * eta_ref;
...
tau_WHSG  = 40.00;             % WHSG time constant                        [s]
...
% Current
n_H2in = n_CH4in * CV;                       % H2 inlet molar flow         [mol/s]
I      = n_H2in * 2 * F * U / N_cell;        % Operating current           [A]

% Molar flows after reforming - Data from Aspen simulations
n_H2Oin  = y_H2O_ref * n_H2in / y_H2_ref;    % H2O inlet molar flow        [mol/s]
n_O2in   = lambda * n_H2in;                  % O2 inlet molar flow         [mol/s]
...
n_H2out  = n_H2in - n_H2r;                   % H2O outlet molar flow       [mol/s]
...
p_H2out  = y_H2out *  p_an;                
p_H2Oout = y_H2Oout *  p_an;               
p_O2out  = x_O2out *  p_cat;

% Partial pressure average
p_H2 = (p_H2in + p_H2out) / 2;
p_H2O = (p_H2Oin + p_H2Oout) / 2;
...
% Concentration at membrane interface [mol/cm3] - Data from [1]
c_O2 = p_O2 / (5.08e06 * exp(-498 / T));
...
% COMBUSTION
Tcomb = 1100;               % Combustion temperature                       [K]
pcomb = p_an;               % Combustion pressure                          [atm]

% THERMAL LOAD
DeltaT_hx = 1;              % DeltaT of the hot water heat exchanger       [K]
T_HWin    = 45 + 273.15;    % Nominal inlet temp. of hot water from TU     [K]

% HEAT EXCHANGER
eta_hx = 0.97;              % Heat exchanger efficiency
...
n_H2Oexh = n_H2Oexh + 2 * n_CH4in * split_CH4;
n_H2Oexh = n_H2Oexh + n_H2out;

% Exhaust CO2 flow [mol / s]
n_CO2in  = y_CO2_ref * n_H2in / y_H2_ref; 
n_CO2exh = n_CO2in + n_CH4in * split_CH4;

% Exhaust N2 flow [mol / s]
n_N2exh  = n_O2in / x_O2 * (1 - x_O2);

% Exhaust flow [mol / s]
n_exh = (n_H2Oexh + n_CO2exh + n_N2exh) * (1 + 0.03);
...
h_CO2comb = refpropm('H', 'T', Tcomb, 'P', pcomb * 100, 'CO2.FLD');
...
h_CO2exh = refpropm('H', 'T', Texh, 'P', pcomb * 100, 'CO2.FLD');
